...
I think there are `r question_interactions %>% left_join(question_details, by = "item_content_id") %>% pull(assessment_item_id) %>% unique() %>% length()` questions in the data.

```{r q5_k, echo = F, warning = F, message = F, error = F, fig.height = 6}
question_interactions %>% 
  left_join(question_details, by = "item_content_id") %>% 
...
```{r q5_density_all30vsNot, echo = F, warning = F, message = F, error = F, fig.height = 6}
question_interactions %>% 
  left_join(question_details, by = "item_content_id") %>% 
