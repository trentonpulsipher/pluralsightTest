...
The range (max - min) of question difficulty (percent correctly answered) for each Assessment:Topic combination is listed below from lowest range to highest. I didn't check the frequency of the questions or of the topics which would affect both the percent correctly answered and possibly the range of difficulty.
...
question_interactions %>% 
  ...
  mutate(label = paste(name, topic_name, sep = ": ")) %>% 
  group_by(label) %>% 
  summarise(range = max(percent_correct)-min(percent_correct)) %>%
  arrange(range)
```
